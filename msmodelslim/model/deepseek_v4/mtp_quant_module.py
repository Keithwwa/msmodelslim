...
class SharedHead(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.norm = NewModelRMSNorm(config.dim, eps=1e-06)
...
class MTPLayer(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.enorm = NewModelRMSNorm(config.dim, eps=1e-06)
        self.hnorm = NewModelRMSNorm(config.dim, eps=1e-06)
        self.e_proj = nn.Linear(config.dim, config.dim, bias=False)
        self.h_proj = nn.Linear(config.dim, config.dim, bias=False)

        self.head = nn.Linear(config.dim, config.vocab_size, bias=False)
        self.norm = NewModelRMSNorm(config.dim, eps=1e-06)

        self.emb = NewModelEmbedding(config.vocab_size, config.dim)
...
def get_mtp_layer(config, model_path, layer_prefix):
    get_logger().debug('Start to load mtp')
    mtp_layer = MTPLayer(config)

    exclude = ['emb.tok_emb.weight', 'head.weight']
    mtp_state_dict = get_state_dict(model_path, mtp_layer, prefix=layer_prefix, exclude=exclude)
    # get shared weights for mtp_layer from main model part
    mtp_state_dict['emb.tok_emb.weight'] = get_shared_weight(model_path, 'embed.weight')
    mtp_state_dict['head.weight'] = get_shared_weight(model_path, 'head.weight')

    auto_dequant_state_dict(layer_prefix, mtp_state_dict, model_path)
    get_logger().debug('Success to convert mtp_layer from fp8 to bf16')
    mtp_layer.load_state_dict(mtp_state_dict)
