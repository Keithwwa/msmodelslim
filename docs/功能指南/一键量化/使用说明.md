...
请参考[安装指南](../../安装指南.md)安装 msModelSlim 工具
...
| device            | 量化设备      | 可选                | 1. 类型：Str <br>2. 参考值：'npu','npu:0,1,2,3','cpu' <br>3. 默认值为"npu"（单设备）<br>4. 指定多个设备时（如：'npu:0,1,2,3'），系统启动DP逐层量化，请确定配置的算法是否支持分布式执行，可以参考[分布式逐层量化特性说明](./分布式逐层量化特性说明<br/>)|
...
## 相关资料
- 对于过大的模型，可以参考[一键量化的逐层量化特性说明](逐层量化特性说明)使用逐层量化，能够明显降低显存使用。
