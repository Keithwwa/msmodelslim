...
- **初学者**：建议优先使用《[一键量化 (V1)](../user_guide/feature_guide/quick_quantization_v1/usage.md)》，它会自动集成合适的算法组合。
- **敏感层与回退**：在定稿 YAML 前可用《[敏感层分析](../user_guide/feature_guide/sensitive_layer_analysis/usage.md)》结合上表 metrics 做层/结构排序；`linear`可首选**Kurtosis**，`layer`可优先**mse_layer_wise**。
